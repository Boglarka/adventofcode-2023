  ...
  for ($i = 0; $i < sizeof($lines); $i++) {
    ...
    for ($j = 0; $j < sizeof($line); $j++) {
      if (is_numeric($line[$j])) {
        ...
        if (!$found) {
          for ($k = -1; $k < 2; $k++) {
            if (($i+$k) >= 0 && ($i+$k) < sizeof($line)) {
              $check_line = str_split($lines[$i+$k]);
              for ($l = -1; $l < 2; $l++) {
                if (($j+$l) >= 0 && ($j+$l) < sizeof($lines) && $check_line[$j+$l] != "." && !is_numeric($check_line[$j+$l])) {
                  $found = true;
                  break;
                }
              }
...
function day2($input) {
  ...
  for ($i = 0; $i < sizeof($lines); $i++) {
    ...
    for ($j = 0; $j < sizeof($line); $j++) {
      if ($line[$j] == "*") {
        for ($k = -1; $k < 2; $k++) {
          if (($i+$k) >= 0 && ($i+$k) < sizeof($lines)) {
            $check_line = str_split($lines[$i+$k]);
            for ($l = -1; $l < 2; $l++) {
              if (($j+$l) >= 0 && ($j+$l) < sizeof($line) && is_numeric($check_line[$j+$l])) {
                $num .= $check_line[$j+$l];
                for ($m = -1; ($j+$l+$m) >= 0; $m--) {
                  if (is_numeric($check_line[$j+$l+$m])) {
                    $num = $check_line[$j+$l+$m] . $num;
                  } else {
                    break;
                  }
                }
                for ($m = 1; ($j+$l+$m) < sizeof($check_line); $m++) {
                  if (is_numeric($check_line[$j+$l+$m])) {
                    $num .= $check_line[$j+$l+$m];
                  } else {
                    $l += $m;
                    break;
                  }
